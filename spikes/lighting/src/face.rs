//! The packed face: 8 bytes, two little-endian `u32`s. The meshing spike's
//! format, with the reserved bits now holding ambient occlusion.
//!
//! ```text
...
//!         bits 23-30 ambient occlusion: 2 bits per corner, corner c = cu | cv << 1
//!                    at bits 23 + 2c, the count of occluders (0..3)
//!         bit  31    reserved, 0
//! ```
//!
...
pub struct Face {
    ...
    pub w: u8,
    pub h: u8,
    /// Occluder count per corner, indexed `cu | cv << 1`.
    pub ao: [u8; 4],
    pub state: u16,
    pub quad: u16,
}
...
impl Face {
    pub fn unit(pos: [u8; 3], dir: u8, state: u16, ao: [u8; 4]) -> Face {
        Face {
            pos,
            dir,
            w: 1,
    ...
    pub fn pack(self) -> [u32; 2] {
        ...
        for (c, ao) in self.ao.iter().enumerate() {
            a |= u32::from(*ao) << (23 + 2 * c);
        }
    ...
    pub fn unpack([a, b]: [u32; 2]) -> Face {
        ...
        Face {
            ...
            w: nib(15) + 1,
            h: nib(19) + 1,
            ao: [0, 1, 2, 3].map(|c| ((a >> (23 + 2 * c)) & 3) as u8),
...
mod tests {
    ...
    fn pack_round_trips() {
        ...
        assert_eq!(Face::unit([0, 0, 0], 0, 1, [0; 4]).pack(), [0, 1]);
    }
}
