...
use crate::world::{Rules, World, props_table};
...
impl Mesher {
    ...
    pub fn new(
        ...
        Mesher {
            ...
            props: gpu.buffer_with(&props_table()),
    ...
    pub fn encode(&self, cb: &gpu::CommandBuffer, jobs: &[u32]) {
        ...
        unsafe {
            ...
            if !jobs.is_empty() {
                ...
                enc.setBuffer_offset_atIndex(Some(&self.props), 0, 11);
    ...
    pub fn read_faces(&self) -> Vec<Vec<Face>> {
        ...
        meshes
            ...
            .map(|m| {
                ...
                for f in &faces[at..at + m.translucent as usize] {
                    out.push(Face::unpack(*f));
                }
                out.sort();
                out
            })
