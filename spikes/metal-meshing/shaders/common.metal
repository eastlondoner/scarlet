...
// The two triangles (0,0)(1,0)(0,1) and (0,1)(1,0)(1,1) are then
// counter-clockwise seen from outside. A greedy face's width runs along u and
// its height along v. Axis numbers: 0 x, 1 y, 2 z.
...
struct SectionInfo {
    ...
    uint neighbour[6];    // section index in each direction, NONE = air
    uint pad[2];
};
...
struct AllocState {
    ...
    uint required;             // largest bump that was asked for (for regrowth)
    uint pad;
};
...
// The ~64 bytes the CPU writes per frame (plus the camera, which could be
// derived from the inverse of view_proj; kept explicit for clarity).
struct Uniforms {
    float4x4 view_proj;
    float4 camera;
};
...
struct CullParams {
    uint section_count;
    uint dir_cull;    // skip directions whose faces all point away from the camera
    uint compact;     // 1: pack visible draws at the front and count them
    uint indexed;     // 1: 4 vertices per face through a shared index buffer
    uint chunk_shift; // chunked path: log2 of faces per instance
    uint chunk_capacity; // chunked path: entries the chunk list holds
    uint pad[2];
};

// Chunked path: one instance per run of up to 2^chunk_shift faces, 8 bytes:
...
//      19-22 height-1, 23-31 reserved (0)
//   y: bits 0-15 block state, 16-31 model quad index (0 = the cube face)
inline uint2 pack_face(uint3 p, uint dir, uint w, uint h, uint state, uint quad) {
