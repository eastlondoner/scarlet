...
8. Floats with the no-NaN rule. **Done** (`float.rs`): every Float goes through `Value::float`, so none is NaN or infinite; `x / 0.0` is `0.0`, `x % 0.0` is `x`, and an answer too large stops at the largest Float. The operators whose operand type is only "a number" (`Add`, `Lt`, ...) pick Int or Float when they run. `scarlet/float`'s maths built-ins, `sin`, `cos`, `tan`, `atan2` and `sqrt`: **built**, through `Value::float` too. The first four are the `libm` crate's, a port of musl's, so every machine gives the same bits; `sqrt` is correctly rounded everywhere, so it is the standard library's. `float.sqrt`'s `Err(Nil)` for a negative is a check in Scarlet, so the VM builds no `Result`.
...
- From `docs/semantics.md`: `x % 0`, and float literals too large to represent.
