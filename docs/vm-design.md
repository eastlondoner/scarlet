...
- A `match` or `if` whose value is used rather than returned is a join (`LetJoin`), and Perceus drops inside it as anywhere else: each branch gives up what it owns and the code after the join does not read. Nothing runs between a branch's value and that code, so a local the value reads for the last time cannot be dropped after it; its reference is handed to the join's result instead (`Atom::Move`, the VM's `Take`), with the value bound to a fresh local first when it is not the local itself. This is Lean's and Koka's rule that a jump to a join point consumes its argument.

This matters because Perceus decides what to drop by type, and it leaves two types alone on purpose: `Int` and `Float` (`ResolvedPool::is_heap`). A Float is always a value word and an Int nearly always is, so a `Drop` of each would be a dispatch for nothing. Under this split they are still freed, just at frame exit: a big `Int` is a heap object Perceus never drops, and it's still correct. A `String` is always a cell and a generic type variable may be one, so both are dropped; a `Drop` of a value word gives up nothing.
...
Each step is one PR or a few. Each PR removes the `#[ignore]` from exactly the tests it makes pass, so `cargo test -p scarlet -- --ignored` counts what's left: 311 after step 2, 301 after step 4, 300 after step 5, 294 once Bool and Nil stopped being constructors, 293 once constructors were built, 248 once `match` ran, 227 once closures did, 222 once tuples did, 219 once arrays were built, 213 once `xs[i]` ran, 212 once slices returned a `Result`, 208 once ranges ran, 204 once `==` did, 190 once the first built-ins ran, 180 once floats did, 174 once binaries did, 165 once the rest of `scarlet/binary` did, 163 once `string`'s built-ins did, and 160 once the bitwise ones did. (Each count is one higher than first written: one Linux-only network test, which skips itself on a Mac, was parked late.)
...
6. Constructors, tuples, fields, then Perceus's `Drop` and reuse. The allocation-count tests come back here. Constructors are built, freed and printed: **done**. A constructor with fields is a heap cell, and one without is a value word of its own, like `True`. `match` on constructors, Ints, Floats and strings, its jumps between cases (`LetCont`/`Goto`), and reading a record's field: **done**. The instruction never checks the variant: a field is found by name (`docs/semantics.md`, "Fields"), and when the variants hold it in different slots the compiler reads it through a `match` on the variant instead. A `..base` spread reads the fields it leaves out the same way. Each arm is a test that jumps to the next arm when the value doesn't fit. Tuples: **done**, a cell of their elements. Perceus's reuse: **built**. A `Drop` the compiler paired with a later constructor of the same shape gives up what its cell holds and keeps the allocation, which that constructor overwrites rather than allocating; `scarlet/internal`'s `cells_made` and `cells_reused` count both. A call to self in tail position keeps those cells, so a loop can take the one its last turn gave up: a 1000-turn loop over a two-field constructor allocates once instead of 1000 times, and that loop runs about a quarter faster. It waits on the last reference, so it fires wherever one exists today — which is loops, not yet a walk over a list, because a call shares its argument and gives it up only after returning: the caller's reference is still there when the callee drops its own. **Moves on last use**: a call followed by the `Drop` of one of its own arguments hands the callee the reference the register holds rather than adding one, so the callee is the only holder and may overwrite the cell. The `Drop` stays where it is and finds the register empty, so a jump that lands on it without the call having run still releases what is there. With it, a walk that rebuilds a list overwrites the list it reads, allocating nothing: `add_one` over 1000 `Cons` cells makes 0 and reuses 1000, and 300 passes over 3000 cells run in 0.05 s rather than 0.09 s.
7. Closures that capture, and calling a function value. **Done.** A closure with captures is a heap cell (its function, then its captures); one without is a value word. Each frame holds the closure it runs as, so a lambda calling itself keeps its captures. What the parked tests stop at next: arrays (33), binaries (31), `wire` (17), processes (about 24), and floats (13).
8. Floats with the no-NaN rule. **Done** (`float.rs`): every Float goes through `Value::float`, so none is NaN or infinite; `x / 0.0` is `0.0`, `x % 0.0` is `x`, and an answer too large stops at the largest Float. The operators whose operand type is only "a number" (`Add`, `Lt`, ...) pick Int or Float when they run.
9. The rest of strings, then binaries (with binary patterns), arrays and maps. Binaries: **built** (`binary.rs`), as the old VM had them: bits counted from the top of the first byte, literals of Int, binary and UTF-8 segments, patterns, and `<<1, 2>>` printing. A slice, from `binary.slice_bits` or a pattern's `rest:binary`, is a cell pointing into the binary it came from, so it copies nothing, as the stdlib promises; a slice of a slice points at the original. `scarlet/binary`'s built-ins all run; the ASCII ones read whole bytes only, as the old VM's did. A `Radix` reaches the VM through `Program::abi`, which the compiler fills by name when the program loads `scarlet/binary`, so nothing reads a constructor's name at run time. Still to come for them: sharing big ones between processes. **Decided:** an array stays the RRB tree the old VM had (`seq.rs` on master): fast at both ends, fast to index, join and slice. Scarlet code uses arrays like lists (walking with `[h, ..t]`, appending in `array.map`, prepending in `reverse`), and only the tree is fast at all three. `[h, ..t]` should give a view of the same tree rather than a copy. Arrays are **built** (`array.rs`): building, length, walking, pushing at either end, joining and `xs[i] or d`, every operation copying only the path it touches. `xs[i]` builds an `Option`: the `Program` names the stdlib constructors the VM builds itself (`Program::abi`: `Some`, `None`, `Ok` and `Err`), as the old VM's `AbiSlot` table did. `xs[a..b]` returns `Result(Array(a), Nil)`, `Err(Nil)` when the range is out of bounds (`docs/semantics.md`, "Arrays"): **built**. A range `a..b` stores only its two ends, as the old VM's did: its length, elements, tail and slices come from them, and adding to one or joining it builds it as a tree first (`array::Seq`). **Built.** `==` is structural, as the old VM's was (`eq.rs`): the same number, string or constructor with equal fields, arrays and tuples with equal elements, a range equal to the array of its Ints, the same function over equal captures. It walks a list of pairs, not the stack. **Built.** `[h, ..t]` is cheap: an array root counts the items left behind at the front of its head leaf, so dropping from the front keeps the same leaf, and a spent head is refilled by moving the tree's leftmost leaf up whole. Walking an array cuts the tree once a leaf rather than once an element, and copies nothing; summing 300,000 elements one at a time went from 0.66 s to 0.20 s. Still to come: the same for a `..base` view. Maps: **built** (`map.rs`), a persistent hash array mapped trie in the CHAMP form (Steindorfer & Vinju, 2015). A `set` or `delete` copies only the nodes on the path to its key, about 4 for a million entries, and shares the rest with the map it came from. CHAMP's shape depends only on what the map holds, so two equal maps have the same nodes: `==` compares them node by node, and they list their entries in the same order. A key's hash (`hash.rs`) reads the value as `==` does, so `0..3` finds `[0, 1, 2]` and `-0.0` finds `0.0`. It is SipHash-1-3 with a fixed key: fixed so the order is the same on every run, and SipHash so that knowing the key does not help anyone make many keys share a whole 64-bit hash, the one case where a map slows down. The old VM hashed with FNV-1a. `os.env` is one of these maps too (step 12).
10. The rest of the intrinsics, one stdlib module at a time. A built-in the VM runs is a `Builtin` instruction, and `code::built` lists which: so far `string.inspect`, `string.length`, `array.length` and `int.to_string`. A function calling any other still loads, and stops when it runs.
11. Processes: mailboxes, the scheduler, preemption, then links, monitors and supervisors.
12. IO (files, sockets, TLS, HTTP), `os`, and `wire` after its redesign. Files, the clocks, random bytes and `os`: **built**. What a run sees of the world outside, its arguments, environment and clock, is a `Host` the driver makes and hands to `scarlet_vm::run`, so a test can hand in its own. A file read or write runs on the one thread and waits for the OS: with no scheduler yet there is nothing else to run. When processes come, these move off the scheduler's threads, as the old VM's blocking pool had them. The OS errors `scarlet/io.IoError` names reach the VM through `Program::abi`, as `Radix` does. `os.env` is a plain map, made the first time a run asks for it and shared after: nothing in a Scarlet program can change its environment, so a map read once says the same as the old VM's live view did, and a map has one form in the VM rather than two. **Open:** backing it by the live environment instead, which is what it is. `scarlet/crypto`: **built**. **Decided:** every crypto primitive is a binding to a well-trusted library, never our own: the digests, HMAC, `const_eq` and signature checks are aws-lc's (`aws-lc-rs`, the provider rustls uses), and random bytes the OS's (`getrandom`). The stdlib's SHA-1, once written in Scarlet, is aws-lc's too. A digest or tag is over bytes, so a binary that is not whole bytes has none, `Err(Nil)`; `const_eq` compares bits, where the old VM dropped a last partial byte and called `<<1, 2:4>>` and `<<1, 3:4>>` equal. `scarlet/json`: **built** (`json.rs`), the old VM's design. `simd-json` parses a document into a compact tape of 16-byte nodes and a string arena, both binaries inside an opaque `Doc`, and every read walks the tape in place (`binary::word` reads a node's two words as they are in the binary's cell), so reading six fields of a large payload copies six strings and nothing else. `Doc`, `ParseError` and `Json`'s constructors reach the VM through `Program::abi`. An `Int` has no bounds, so every integer the parser takes is one, where the old VM's `json.int` refused anything past `i64::MAX`. **Open:** an integer past 64 bits does not parse at all, since `simd-json` has no wider integer. `scarlet/http/h1`'s parsing and `scarlet/http/headers`' lookups: **built** (`http.rs`). A head is read by `httparse`, the parser `hyper` uses, rather than the old VM's own scanner: **decided**, for the same reason crypto is bound rather than written. What the stdlib promises beyond that parse is ours, around it: CRLF-only lines, at most four empty lines before a request and none before a response, the 64 KiB cap, which status each refusal is, the `Connection` and `Expect` tokens, a body's framing, and the chunked-body loop, whose size lines `httparse` reads. It is stricter than the old scanner in four places, each as RFC 9110 or 9112 allows: a token after the version is 400 (it was 505), a bare LF or CR is 400 at once (the old head never ended and waited out its deadline), and a NUL in a field value is 400 (it parsed). The method, target and fields come back as slices of the buffer the program read, so a head costs its fields' small cells and nothing else; httparse reads a copy of at most 64 KiB of it. The descriptor builder (`typed_ir/wire.rs` on master, removed with the old VM) comes back, and its descriptors reach the VM through `scarlet_ir`: today's Core IR says only `WireDecode(%0)`. **Decided:** a closure on the wire names its code, not its run: its module, its function and a fingerprint of the module's code, as BEAM's `NEW_FUN_EXT` does with the module's MD5. So the same code can decode it after a restart or on another machine, and changed code is a decode error. Handles (`Pid`, `Subject`, sockets) stay tied to the run that made them.
